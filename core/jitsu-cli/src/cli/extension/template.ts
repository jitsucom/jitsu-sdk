...
export const packageJsonTemplate = ({ packageName, type, jitsuVersion = undefined }: TemplateVars) => ({
  ...
  scripts: {
    clean: "rm -rf ./dist",
    build: `${binName} extension build`,
    test: `${binName} extension test`,
    "validate-config": `${binName} extension validate-config`,
    execute: `${binName} extension ${type == "destination" ? "exec" : "exec-src"}`,
  },
  devDependencies: {
    "@jitsu/types": `${jitsuVersion || "^" + jitsuCliVersion}`,
    "@jitsu/jlib": `${jitsuVersion || "^" + jitsuCliVersion}`,
    "ts-jest": "^27.0.7",
    [binName]: `${jitsuVersion || "^" + jitsuCliVersion}`,
    tslib: "^2.3.1",
    typescript: "^4.5.2",
  },
  dependencies: {},
...
let destinationTest = ({ type = "destination" }) => {
  ...
  return `
    ...
    import { testDestination } from "${binName}/lib/tests"
...
descriptor["destination"] = (vars: TemplateVars) => `
  ...
  const descriptor: ExtensionDescriptor = {
    ...
    icon: "",
    description: "Jitsu destination - ${vars.packageName} (generated by \`${binName} extension create\`)",
    configurationParameters: [
...
descriptor["transform"] = (vars: TemplateVars) =>
  ...
  const descriptor: DestinationDescriptor = {
    ...
    displayName: "${vars.packageName}",
    icon: "",
    description: "Jitsu ${vars.type} - ${vars.packageName} (generated by \`${binName} extension create)\`",
    configurationParameters: [
